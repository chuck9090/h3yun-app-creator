...
# ---------------------------------------------------------------- 看板/报表(非业务实体)
# 看板/报表/统计/分析类是对业务数据的**统计与展示**,不是业务实体:
#   - 不进「业务流程图」(它们不在业务流转链上);
#   - 不进「ER 结构」,因此也**不会生成氚云应用**(由氚云端 SQL 报表/高级数据源另配)。
#
# 判定分两档,**避免误伤真实业务表**(如「质量分析单」「设备监控台账」):
#   强信号:出现即判定为报表(这些词几乎只用于展示类);
#   弱信号:仅当**以该词结尾**才算(报表名多以「分析/统计/监控」收尾)。
_STRONG_REPORT = ("看板", "报表", "总览", "图表", "dashboard")
_WEAK_REPORT_SUFFIX = ("分析", "统计", "监控")


def is_report_entity(name) -> bool:
    """名称是否为看板/报表类(用于模块名与表单名)。"""
    n = str(name or "").strip().lower()
    if not n:
        return False
    if any(k in n for k in _STRONG_REPORT):
        return True
    return n.endswith(_WEAK_REPORT_SUFFIX)
