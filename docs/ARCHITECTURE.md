...
| 6 | 点「生成方案」→ 系统设计方案(**客户可交付格式**:标题1=模块/标题2=表单/编号业务内容,不含 key 与界面配置)。**方案要写全字段/规则/关系,是后续唯一依据**;生成时校验需求清单覆盖度 | 方案页 | `POST .../plan/generate` |
| 7 | 打开方案编辑页改 markdown | md 编辑器 | `PUT .../plan` |
| 8 | 点「生成业务流程图」→ **仅依方案**、分模块的 Mermaid 图(方案为空返回 400;看板/报表不进图;支持按模块/平移/缩放) | 流程图页(Mermaid) | `POST .../flowchart/generate` |
| 9 | 不满意→改方案→重生成流程图(循环) | 同上 | 同上 |
| 10 | 点「生成 ER 图」→ **依据「方案 + 业务流程图」**(流程图用于判断自动化);看板/报表不建表 | ER 页(React Flow + 结构化编辑器) | `POST .../design/generate` |
...
并编译进 `data/knowledge/library.md`,供**所有项目**生成**方案**时按**资料名称**勾选参考(`projects.ref_items`)。
> **看板/报表/统计/分析/总览/监控类**:是对数据的展示,不是业务实体 —— **不建表、不进流程图与 ER、不生成应用**;
> 由氚云端 SQL 报表/高级数据源另配。判定:`context.is_report_entity`(强信号子串 看板/报表/总览/图表 + 弱信号结尾 分析/统计/监控)。
...
`data/knowledge/library.md`,供所有项目生成**方案**时按资料名称勾选参考。
...
**参考资料只在「生成方案」时使用**(方案=唯一源头);**业务流程图只依方案,ER 只依「方案 + 业务流程图」**,二者都不再读原始资料。
生成方案时,参考资料 = 本项目文档 + 勾选的资料库资料。为避免"文件一多就塞爆上下文",链路为:
...
阶段内容区不再内嵌进度面板。侧栏明细行:文字可换行、右侧「百分比+时间」不被遮、时间戳只显示时分秒、列表顶部留白防首行被裁。
**其他界面能力**:左侧导航可折叠(偏好存 `localStorage.h3ac.siderCollapsed`,与窄屏自动折叠分离);
右下角「回到顶部」按钮;方案页左侧目录(sticky、点击跳转、滚动高亮);业务流程图的「按模块/全部」切换、拖动平移、缩放。
任务跑在**进程内线程池**(单进程部署假设):
