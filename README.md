...
**依赖是单向的**:《系统设计方案》是**唯一源头**(写全每个表单的字段/业务规则/表单间关系);
**业务流程图只依据方案**,**ER 结构依据「方案 + 业务流程图」**(据流程图推自动化)。
**看板 / 报表 / 统计 / 分析类**是对数据的展示,**不建表、不进流程图与 ER、不生成应用**(由氚云端 SQL 报表另配)。
...
5. 「生成方案」→ 系统设计方案(HTML 展示,底层 markdown),**客户可交付格式**:标题1=模块、标题2=表单、
   ...
   方案要**写全字段/规则/关系**(它是流程图与 ER 的唯一依据);模块按需求清单的「功能模块」分组,
   **生成时会校验清单覆盖度**,缺失项告警(缺失会连带影响流程图与 ER);可进编辑页改;
6. 「生成业务流程图」→ **仅依据方案**的分模块 Mermaid 图(节点=表单、模块分组、表单间流转为边);
   看板/报表已排除;支持**按模块查看、拖动平移、缩放**;不满意 → 回改方案 → 重生成;
7. 「生成 ER 图」→ **依据「方案 + 业务流程图」**的表图谱(流程图用于判断自动化);可改表名、加表、改字段名、控件类型、加字段;
   看板/报表**不建表**(也不会生成应用);
...
│                         方案页左侧目录、回到顶部;任务进度中心(右侧边栏,可收起为悬浮图标,阶段标题显示百分比)
...
- **需求清单驱动设计**:需求清单(每项目限一份)是需求基准,其「功能模块」决定模块划分与顺序,
  列出的表单是**必做项**;**生成方案时**即校验清单覆盖度,缺失项告警。
- **单一依赖链**:方案=唯一源头(写全字段/规则/关系)→ 流程图**只依方案** → ER 依**方案+流程图**。
- **看板/报表不建表**:看板/报表/统计/分析/总览/监控类是对数据的展示,**不进流程图与 ER、不生成应用**
  (由氚云端 SQL 报表/高级数据源另配);判定用「强信号子串(看板/报表/总览/图表)+ 弱信号结尾(分析/统计/监控)」,
  避免误伤「质量分析单 / 设备监控台账」这类业务表。
- **业务流程图以表单为视角**:节点=表单、边=表单间业务关系、按模块分区;支持**按模块/全部切换、拖动平移、缩放**。
- **AI 参考 = 用户资料**:出**方案**的参考资料来自**用户上传的文档**(需求清单/会议纪要/其他)
  与本部署已建项目语料(`data/knowledge/`),**不含任何内置样本**;资料库共享资料作为「外部参考资料」
  只借鉴字段/口径,不复用其模块。流程图与 ER 不再读原始资料(只依方案/流程图)。
