...
5. 「生成方案」→ 系统设计方案(HTML 展示,底层 markdown),**客户可交付格式**:标题1=模块、标题2=表单、
   其下用阿拉伯数字逐项列出业务内容(字段中文名/子表/业务规则/流程);**不含字段 key 与界面配置**。
   模块划分与顺序按需求清单的「功能模块」分组,清单里的表单逐一覆盖(漏项会在生成 ER 时告警);可进编辑页改;
...
│                         页面:登录/项目/工作台/资料库/设置/用户;含主题切换与任务进度中心
│                         (右侧边栏展示详细进度,可收起为右下角悬浮图标;阶段标题显示生成百分比)
...
- **编码规范化**:表单/字段/子表/子表列/自动化编码**只含字母数字、字母开头,不含任何符号**
  (下划线/连字符/点号/中文等自动驼峰化,不丢表);含路径分隔符或 `..` 的危险 key 直接剔除。
- **编码排除保留字**:字段/子表列/表单/子表编码自动避开**氚云平台自带编码**(主表/子表/中间表)
  与 **MySQL 保留关键字**(编码会成为数据库列名/表名,撞上会让 SQL 报表失败);生成侧 `clean_design`
  自动改名并同步重映射引用,build 侧 `h3design.dsl._check_reserved` 兜底拦截。**已建表**(`created: true`)
  编码原样保留,避免另起一列/丢数据。
